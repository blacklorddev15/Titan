...
function addPremium(jid)  {
  ...
  d.premiumUsers.push(n); writeJSON(PREMIUM_FILE, d);
  if (neonDb.enabled) neonDb.addPremiumNumber(n).catch(() => {});
  return true;
}
function removePremium(jid) {
  ...
  if (i === -1) return false;
  d.premiumUsers.splice(i, 1); writeJSON(PREMIUM_FILE, d);
  if (neonDb.enabled) neonDb.removePremiumNumber(n).catch(() => {});
  return true;
}

// Pull any premium numbers stored on Neon into the local list at boot
// (fire-and-forget: never blocks or breaks startup when Neon is offline).
async function syncRemotePremium() {
  if (!neonDb.enabled) return 0;
  const remote = await neonDb.loadPremiumNumbers();
  if (!Array.isArray(remote)) return 0;
  const d = readJSON(PREMIUM_FILE, { premiumUsers: [] });
  const list = Array.isArray(d.premiumUsers) ? d.premiumUsers : [];
  let added = 0;
  for (const num of remote) {
    const n = String(num).replace(/\D/g, '');
    if (n && !list.includes(n)) { list.push(n); added++; }
  }
  if (added) { d.premiumUsers = list; writeJSON(PREMIUM_FILE, d); }
  return added;
}
setTimeout(() => { syncRemotePremium().catch(() => {}); }, 2500);
...
const DEFAULTS = {
  ...
  owner: '', botName: 'TITAN ANIME MD', menuImg: '',
